...
当前发布版本为 `0.3.11`，`main` 分支可能包含尚未发布的改进。
...
- 新增可即时预览并持久化的深色、亮色主题。
- 修复设为默认图片应用后，资源管理器大图标视图可能回退显示 Pixora 图标的问题。
- 设置页“诊断与关于”现在清晰显示当前版本号和程序更新时间。
- 界面美观性优化：标题栏颜色随主题同步，控件悬停/按下有平滑过渡动画，右键菜单和徽标使用统一的系统图标，画布浮层配色完全主题化。
