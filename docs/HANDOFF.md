...
- 当前发布版本：`0.3.11`
- 发布标签：`v0.3.11`
- 发布基线提交：`v0.3.11` 标签所指向的提交。
- `main` 的发布代码已与远端同步；接手时仍须先检查工作区状态。
- 正式下载包：GitHub Release `v0.3.11` 中的 `Pixora-win-x64.zip`。
...
- `Pixora.Image` 会注册 Windows 照片缩略图处理器；已有文件关联由当前有效安装在启动时自动补齐，避免资源管理器大图标视图回退为应用图标。
- 设置页“诊断与关于”显示面向用户的版本号和当前程序文件更新时间；完整构建信息仍保留在复制诊断信息中。
